...
class THSTrader:
    ...
    def get_balance(self):
        ...
        time.sleep(0.5)
        result = {
            "可用余额": self.main_wnd.Static4.window_text()
        }
        result["total"] = float(self.main_wnd.Static6.window_text())
        result["free"] = float(result["可用余额"])
        time.sleep(0.5)
...
#        print(cancelable_entrusts)
    ...
    def __trade(self, stock_no, price, amount):
        ...
        self.app.top_window().set_focus()
        try:
            result = self.app.top_window().window(control_id=0x3EC, class_name='Static').window_text()  # 确定
        except:
            self.app.top_window().window(control_id=0x6, class_name='Button').click()  # 确定买入
    ...
    def __click_update_button(self):
        # self.app.Button0.click()
        self.app.top_window().window(control_id=32790, class_name='Button',found_index=0) .click()
